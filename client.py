...
class client:
    ...
    def generate_public_key(self,roll):
    ...
    def generate_shared_key(self,key1, key2):
    ...
    def DES_encrypt(self,key, plaintext, isFile):
        key = str(key)
        if(len(key)<15):
    ...
    def process_query(self,ClientSocket, cmd):
        tokens = cmd.split(" ")
        keyword = tokens[0]
        if keyword == "LOGIN":
            if loginStatus:
    ...
    def send_file_data(self,iport, filePath):
        #print(filePath)
        header = self.getFileName(filePath) + "\n\n"
        ...
        pubkey, pk = self.generate_public_key(rollno)
        pubkey = str(pubkey)
        pk = str(pk)
        ...
        rkey = int(rkey)
        pk = int(pk)
        sharedKey = self.generate_shared_key(rkey, pk)

        msg = header.encode() + content
        msg = self.DES_encrypt(sharedKey, msg, True)
    ...
    def send_file_to_group(self,iplist, filePath):
        for iport in iplist:
            ip, port = iport.split(":")
            if port==sport:
                continue
            self.send_file_data(iport, filePath)

    #----------------------------------------------------------------------------------------------------------------#

    def init_communication(self,resp):
        tkns = resp[4:].split("||")
        iplist = tkns[:-1]
        #print(iplist)
        ...
        if file_info[0].lower().replace(" ", "") == "file":
            if(len(iplist)>1):
                self.send_file_to_group(iplist, file_info[1])
                return "File Successfully transferred."
            self.send_file_data(iplist[0], file_info[1])
            return "File Successfully transferred."
        for iport in iplist:
            ip, port = iport.split(":")
            msg = userName + " :\> " + tkns[-1]
        ...
        #-------------------- ENCRYPTION--------------------#
            ...
            msg = self.DES_encrypt(sharedKey, msg, False)
    ...
    def send(self,ClientSocket, cmd):
        ...
        if(Response[:2]=="IP"):
            Response = self.init_communication(Response)
        self.setLoginStatus(Response)
        return ("Server :\> " + Response)

    #----------------------------------------------------------------------------------------------------------------#

    def setLoginStatus(self,resp):
    ...
    def init_server(self,IP, PORT):
        ...
        while True:
            ...
            data = self.DES_decrypt(sharedKey, data)
    ...
    def main(self):
        ...
        sport = str(PORT)
        start_new_thread(self.init_server, (IP, PORT))

        while True:
            cmd = input()
            cmd+= " " + iport
            resp = self.process_query(ClientSocket, cmd)
            print(resp)
            print()
        ClientSocket.close()
c1=client()
c1.fetch_server_info()
c1.main()
